...
USE_NEWS_FEATURES = os.environ.get("USE_NEWS_FEATURES", "0") == "1"
...
def _load_news_snapshot(symbol: str, ts_ms: int) -> Dict[str, float]:
    try:
        from engine.storage import connect
        con = connect()
        try:
            bucket_sec = int(os.environ.get("NEWS_BUCKET_SEC", "300"))
            if bucket_sec <= 0:
                bucket_sec = 300
            ...
            row = con.execute(
                """
                SELECT score, decayed_score, count
                FROM news_features
                WHERE symbol=? AND bucket_ts_ms=? AND bucket_sec=?
                """,
                (str(symbol).upper().strip(), int(b), int(bucket_sec)),
            ).fetchone()
            if not row:
                return {"score": 0.0, "decayed_score": 0.0, "count": 0.0}
            return {
                "score": _safe_float(row[0]),
                "decayed_score": _safe_float(row[1]),
...
def build_feature_vector(*, event: Dict, symbol: str) -> list:
    ...
    if USE_NEWS_FEATURES:
        nf = _load_news_snapshot(symbol=str(symbol), ts_ms=int(ts_ms))
        features.extend([
            float(nf.get("score", 0.0)),
            float(nf.get("decayed_score", 0.0)),
            float(nf.get("count", 0.0)),
        ])
