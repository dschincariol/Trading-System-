...
# Soft staleness throttle: if age > 0.5*max, multiply by this
PORTFOLIO_EXEC_STALE_HALF_FACTOR = float(os.environ.get("PORTFOLIO_EXEC_STALE_HALF_FACTOR", "0.50"))
...
def _pick_best_per_symbol(alerts: List[Dict]) -> Dict[str, Dict]:
    ...
    for a in alerts:
        ...
        if conf < PORTFOLIO_MIN_CONF:
            continue
        if abs(z) < PORTFOLIO_MIN_ABS_Z:
            continue
...
def _apply_temporal_dampener(con, desired: Dict[str, Dict], now_ms: int) -> Dict[str, Dict]:
    ...
    if grossT > float(PORTFOLIO_GROSS_CAP) and grossT > 1e-9:
        ...
        for sym in list(desired.keys()):
            desired[sym]["weight"] = float(desired[sym]["weight"]) * float(scaleT)
...
def _apply_impact_aware_sizing(con, desired: Dict[str, Dict]) -> Dict[str, Dict]:
    ...
    if gross > float(PORTFOLIO_GROSS_CAP) and gross > 1e-9:
        ...
        for sym in list(desired.keys()):
            desired[sym]["weight"] = float(desired[sym]["weight"]) * float(sc)

    return desired


def _optimize_capital_allocation(desired: Dict[str, Dict]) -> Dict[str, Dict]:
    """
    ...
    for sym, tgt in desired.items():
        ...
        # conservative: ignore negative expectancy in optimizer (let other logic drop it)
        netp = max(0.0, net)

        # eps prevents div-by-zero; dd^beta keeps consistent with risk aversion
        u = (netp ** max(0.0, alpha)) / ((dd ** max(0.0, beta)) + 1e-6)
    ...
    for sym in list(desired.keys()):
        ...
        # (gross re-normalization happens later)
        # baseline target: w_t = gross_cap * share (bounded)
...
def _write_state_row(con, sym: str, side: str, weight: float, opened_ts_ms: int,
                     updated_ts_ms: int, source_alert_id: Optional[int], explain_json: str) -> None:
    con.execute(
        """
...
def _apply_capital_at_risk_gate(desired: Dict[str, Dict]) -> Tuple[Dict[str, Dict], Dict]:
    ...
    """
    meta = {"car_enabled": True, "car_max": float(PORTFOLIO_CAR_MAX), "car_max_per_symbol": float(PORTFOLIO_CAR_MAX_PER_SYMBOL)}
    if not desired:
    ...
    for sym, tgt in desired.items():
        ...
        if float(PORTFOLIO_CAR_MAX_PER_SYMBOL) > 0.0:
            max_w = float(PORTFOLIO_CAR_MAX_PER_SYMBOL) / max(1e-9, dd if dd > 0 else 1.0)
            if w > max_w:
                new_w = float(max_w)
                # preserve sign through side
                tgt["weight"] = float(new_w) if tgt.get("side") != "SHORT" else float(new_w)
    ...
    if grossC > float(PORTFOLIO_GROSS_CAP) and grossC > 1e-9:
        ...
        for sym in list(desired.keys()):
            desired[sym]["weight"] = float(desired[sym]["weight"]) * float(scaleC)
...
def _emit_order(con, sym: str, action: str, from_side: str, to_side: str,
                from_w: float, to_w: float, source_alert_id: Optional[int], explain: Dict) -> None:
    ts_ms = _now_ms()
...
def compute_rebalance() -> Dict:
    ...
    Returns a summary dict (safe to expose in /api).
    """
    init_portfolio_db()
    ...
    try:
        ...
        if last:
            try:
                last_ms = int(last)
                if (now_ms - last_ms) < int(PORTFOLIO_REBALANCE_COOLDOWN_S) * 1000:
                    return {"ok": False, "error": "rebalance cooldown active", "cooldown_s": PORTFOLIO_REBALANCE_COOLDOWN_S}
            except Exception:
                pass
        ...
        strategy_name = choose_strategy_name(con, now_ms=int(now_ms))
        strat = load_strategy_module(strategy_name)

        # optional RL metadata (strategy may expose it)
        rl_score = None
        try:
            rl_score = getattr(strat, "last_rl_score", None)
        except Exception:
            rl_score = None

        # strategy builds desired targets
        desired = strat.build_desired(alerts=alerts, now_ms=int(now_ms)) or {}
        ...
        try:
            for sym in list(desired.keys()):
                ...
                if nlab < int(PORTFOLIO_EXPLORE_MIN_LABELS):
                    ...
                    desired[sym]["weight"] = float(_clamp(desired[sym]["weight"], 0.0, float(PORTFOLIO_EXPLORE_MAX_W)))
        ...
        # hard cap max positions (keep largest abs weights)
        if len(desired) > int(PORTFOLIO_MAX_POSITIONS):
        ...
        try:
            ...
            if PORTFOLIO_USE_VOL_TARGET:
                ...
                if gross2 > float(PORTFOLIO_GROSS_CAP) and gross2 > 1e-9:
                    ...
                    for sym in list(desired.keys()):
                        desired[sym]["weight"] = float(desired[sym]["weight"]) * float(scale2)
        except Exception:
            pass
        ...
        try:
            if PORTFOLIO_USE_STRESS_GATE and desired:
                st = _vix_stress(con)
                f_stress = float(_stress_factor_from_vix_z(float(st.get("z", 0.0))))

                if f_stress < 1.0:
                    for sym in list(desired.keys()):
                        desired[sym]["weight"] = float(desired[sym]["weight"]) * float(f_stress)
                        desired[sym].setdefault("reason", {})
                        desired[sym]["reason"]["stress_gate_factor"] = float(f_stress)
                        desired[sym]["reason"]["stress_vix_level"] = float(st.get("level", 0.0))
                        desired[sym]["reason"]["stress_vix_z"] = float(st.get("z", 0.0))
                        desired[sym]["reason"]["stress_vix_change_1"] = float(st.get("change_1", 0.0))

                # renormalize gross after stress compression (still respect gross cap)
                gross_s = sum(abs(float(v.get("weight", 0.0))) for v in desired.values())
                if gross_s > float(PORTFOLIO_GROSS_CAP) and gross_s > 1e-9:
        ...
        try:
            if PORTFOLIO_USE_VOV_GATE and desired:
                ...
                if compute_tech_features:
                    ...
                    for sym in list(desired.keys()):
                        tf = compute_tech_features(str(sym), int(now_ms)) or {}
        ...
        if PORTFOLIO_USE_EXEC_REALISM:
            for sym in list(desired.keys()):
                try:
                    ...
                    ef, meta = 1.0, {"staleness_sec": 0.0, "stress_vix_z_60": 0.0, "atr_pct": 0.0, "slippage_bps_est": 0.0}
        ...
        for sym, tgt in desired.items():
            ...
            explain = {
                ...
                "selector": {
                    "rule_choice": str(_get_meta(con, "last_rule_choice") or strategy_name),
                    "rl_choice": str(_get_meta(con, "last_rl_choice") or strategy_name),
        ...
        for sym, cur in state.items():
            ...
            if from_side in ("LONG", "SHORT") and from_w > 0:
                ...
                if age_s < float(PORTFOLIO_MIN_HOLD_S):
                    explain = {"hold_reason": f"min_hold not met for close (age_s={age_s:.1f} < {PORTFOLIO_MIN_HOLD_S})"}
        ...
        try:
            ...
            if gross_now > peak:
                peak = gross_now
            drawdown = (gross_now - peak) if peak > 1e-9 else 0.0
        ...
        _set_meta(con, "last_strategy_name", str(strategy_name))
