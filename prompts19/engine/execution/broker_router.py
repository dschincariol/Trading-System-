...
# Tradable unit normalization (symbol, instrument_type, venue)
try:
    from engine.execution.tradable_unit import normalize_orders as _normalize_tradable_orders  # type: ignore
except Exception:
    _normalize_tradable_orders = None  # type: ignore

# Learnable venue selection (profitability-tied)
try:
    from engine.execution.venue_selection import choose_best_venue as _choose_best_venue  # type: ignore
except Exception:
    _choose_best_venue = None  # type: ignore
...
def _parse_venue_candidates(broker_name: str) -> List[str]:
    b = str(broker_name or "").lower().strip()

    # Broker-specific override
    env_key = f"EXEC_VENUE_CANDIDATES_{str(b).upper()}"
    raw = str(os.environ.get(env_key, "") or "").strip()
    if raw:
        return [p.strip().upper() for p in raw.split(",") if p.strip()]

    # Global fallback
    raw2 = str(os.environ.get("EXEC_VENUE_CANDIDATES", "") or "").strip()
    if raw2:
        return [p.strip().upper() for p in raw2.split(",") if p.strip()]

    # Safe defaults per broker
...
def _apply_venue_selection(
    *,
    orders: List[dict],
    broker_name: str,
) -> List[dict]:
    """Best-effort venue selection.

    Rules:
    - Disabled unless EXEC_VENUE_SELECTION=1.
    - Does not override an explicit order['venue'] unless EXEC_VENUE_OVERRIDE=1.
    - Fail-open: any exception keeps the original venue.
    """
    if os.environ.get("EXEC_VENUE_SELECTION", "0") != "1":
        return orders

    if not callable(_choose_best_venue):
        return orders

    override = os.environ.get("EXEC_VENUE_OVERRIDE", "0") == "1"
    lookback_days = int(os.environ.get("EXEC_VENUE_LOOKBACK_DAYS", "14"))
    min_n = int(os.environ.get("EXEC_VENUE_MIN_N", "25"))

    candidates = _parse_venue_candidates(broker_name)
    if not candidates:
        return orders
    ...
    for o in list(orders or []):
        ...
        it = str(o.get("instrument_type") or "").upper().strip()
        cur_venue = str(o.get("venue") or "").upper().strip()

        if not sym or not it:
            out.append(o)
            continue

        if cur_venue and (not override):
            out.append(o)
            continue

        try:
            chosen, meta = _choose_best_venue(
                symbol=sym,
                instrument_type=it,
                candidates=list(candidates),
                lookback_days=int(lookback_days),
                min_n=int(min_n),
                default_venue=(cur_venue or (candidates[0] if candidates else None)),
            )
        except Exception:
            out.append(o)
            continue
        ...
        no = dict(o)
        if chosen:
            no["venue"] = str(chosen).upper().strip()
        no["venue_selection"] = meta
        out.append(no)
...
def _apply_one(
    ...
    *,
    ...
    # Best-effort normalization for venue + instrument_type fields.
    # This is intentionally fail-open to preserve legacy payload compatibility.
    if override_orders and callable(_normalize_tradable_orders):
        try:
            override_orders, _ = _normalize_tradable_orders(list(override_orders or []), broker=name)
        except Exception:
            pass

    # Learnable venue selection (best-effort).
    if override_orders:
        try:
            override_orders = _apply_venue_selection(orders=list(override_orders or []), broker_name=name)
        except Exception:
            pass
