...
def get_ops_snapshot(jobs=None):
    """
    Build a normalized operations snapshot used by AI Ops Copilot.
    Combines the standard health snapshot with job status, drift metrics,
    validation scores and simple confidence statistics.

    `jobs` should be the result of JOBS.list_jobs(); if omitted an empty list
    will be used.
    """
    if jobs is None:
        jobs = []
    ...
    # --- drift metrics --------------------------------------------------
    try:
        from engine.strategy.drift_utils import get_max_drift_ratio
        con = _db_connect()
        try:
            rows = con.execute(
                "SELECT symbol, drift_ratio FROM model_drift ORDER BY drift_ratio DESC LIMIT 5"
            ).fetchall()
            top = [
                {"symbol": r[0], "drift_ratio": float(r[1])} for r in rows
            ]
        except Exception:
            top = []
        finally:
            try:
                ...
                pass
        snap["drift"] = {"max_ratio": get_max_drift_ratio(), "top": top}
    except Exception:
        snap["drift"] = {"max_ratio": 0.0, "top": []}
    ...
    try:
        ...
        try:
            rows = con.execute(
                "SELECT symbol, horizon_s, mae, rmse, n FROM validation_scores"
            ).fetchall()
            snap["validation"] = [
                {"symbol": r[0], "horizon_s": r[1], "mae": float(r[2]), "rmse": float(r[3]), "n": int(r[4])}
                for r in rows
            ]
        except Exception:
            snap["validation"] = []
    ...
    try:
        ...
        try:
            row = con.execute(
                "SELECT AVG(confidence), MIN(confidence), MAX(confidence) FROM temporal_predictions WHERE ts_ms>=?",
                (cutoff,),
            ).fetchone()
            if row:
                snap["confidence"] = {
                ...
                snap["confidence"] = {"avg": 0.0, "min": 0.0, "max": 0.0}
        except Exception:
            snap["confidence"] = {"avg": 0.0, "min": 0.0, "max": 0.0}
        finally:
            try:
