...
from engine.news_domain import (
    extract_domain,
    is_domain_blocked,
    domain_conf_multiplier,
    is_source_blocked,
    source_conf_multiplier,
)
...
def _tradability_from_pred(
    ...
    *,
    ...
    # Dynamic execution cost estimate (best-effort):
    # - Prefer learned execution cost model when available
    # - Fall back to static COST_BPS otherwise
    expected_cost = float(COST_BPS) / 10000.0
    if con is not None and callable(estimate_cost_bps) and symbol:
        spread_bps = None
        try:
            r = con.execute(
                """
            ...
            if r:
                ...
                if spr is None and bid is not None and ask is not None:
                    spr = float(ask) - float(bid)
                if last is not None and float(last) > 1e-9 and spr is not None:
                    spread_bps = float(spr) / float(last) * 10000.0
        except Exception:
            spread_bps = None
        ...
        try:
            ...
            if isinstance(c, dict) and c.get("total_cost_bps") is not None:
                expected_cost = float(c.get("total_cost_bps") or 0.0) / 10000.0
        except Exception:
            pass
    ...
    try:
        ...
        try:
            ...
            for (eid, ts_ms, source, title, body, url, meta_json), vec in zip(rows, embeddings):
                ...
                # early quarantine check: if ingestion already flagged the event
                # because the source was blacklisted, do not even attempt to
                # generate embeddings/predictions.  this keeps the "bad" data
                # out of every downstream dataset while still preserving it for
                # forensic analysis.
                if event_meta.get("quarantine"):
                    logging.info("skipping quarantined event id=%s source=%s", eid, source)
                    # update checkpoint so we don't loop forever
                    ck["last_event_id"] = int(eid)
                    ck["last_event_ts_ms"] = int(ts_ms)
                    continue

                domain = extract_domain(url, meta_json)
                if domain:
                    event_meta["domain"] = domain
                # record original source for downstream scoring/blacklisting
                if source:
                ...
                for sym in symbols:
                    ...
                    for h in HORIZONS:
                        ...
                        # quietly skip symbols for which either the domain or source
                        # has been blacklisted.  events still land in the database but
                        # no predictions/alerts are generated (quarantine).  This mirrors
                        # existing domain logic and adds a source-level companion.
                        if domain and is_domain_blocked(domain, sym):
                            continue
                        if source and is_source_blocked(source, sym):
                            continue
                        if domain:
