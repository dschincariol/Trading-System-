...
def _load_prices(con, symbol: str, ts_ms: int, n: int) -> List[Tuple[int, float]]:
    # exclude the boundary timestamp to prevent using price at event time (potential lookahead)
    rows = con.execute(
        """
        SELECT ts_ms, price
        FROM prices
...
def run_once(ts_ms: Optional[int] = None) -> None:
    ...
    with connect() as con:
        ...
        # --- news features: bring latest decayed score into factor universe
        try:
            rows = con.execute(
                """
                SELECT symbol, decayed_score
                FROM news_features
                WHERE bucket_ts_ms = (
                    SELECT MAX(bucket_ts_ms)
                    FROM news_features
                    WHERE bucket_ts_ms <= ?
                )
                """,
            ...
            for sym, val in rows or []:
                put_factor_feature(
                    con,
                    feature_id="news.decayed_score",
                    asof_ts=ts_ms,
                    effective_ts=ts_ms,
