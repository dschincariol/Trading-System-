...
def compute_provider_health():
    ...
    # make sure diagnostics table exists
    try:
        con.execute(
            """
            CREATE TABLE IF NOT EXISTS provider_degraded(
              ts_ms INTEGER,
              provider TEXT,
              stale_ms INTEGER
            )
    ...
    try:
        ...
        for provider, last_ts in rows:
            ...
            # provider degradation: if score drops too low mark DOWN
            if score < 0.2:
                status = "DOWN"
                score = 0.0
                # log degradation event for later analysis
                try:
                    con.execute(
                        """
                        INSERT INTO provider_degraded(ts_ms, provider, stale_ms)
                        VALUES (?,?,?)
                        """,
                        (ts, provider, stale),
                    )
