...
def realized_vol_proxy(con, symbol: str, ts_ms: int, lookback_points: int = 50) -> float:
    rows = con.execute(
        """
        SELECT price
        FROM prices
        ...
        LIMIT ?
        """,
        (symbol, int(ts_ms), int(lookback_points)),
    ).fetchall()
...
def label_due_events_internal() -> int:
    ...
    try:
        ...
        for eid, ets, _title in events:
            for sym in SYMBOLS:
                vol = realized_vol_proxy(con, sym, int(ets), lookback_points=80)
