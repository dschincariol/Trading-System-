...
from engine.data.ingest.rss_ingest import _normalize_entities
    ...
    *,
    ...
    for sym, q in queries.items():
        ...
        try:
            ...
            arts.extend((j or {}).get("articles") or [])
    ...
    for a in arts:
        try:
            ...
            meta = {
                "provider": "gdelt",
                "matched_symbol": sym,
                "entities": _normalize_entities(f"{title}\n"),
                "gdelt": {
            ...
            # quarantine flag if the feed itself is already blocked
            if is_source_blocked(src, "*"):
