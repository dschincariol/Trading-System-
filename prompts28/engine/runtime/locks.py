...
def cleanup_stale_locks():
    """Purge expired job locks from the database.

    This can be called at startup or periodically by a background thread
    to ensure orphaned locks (due to crashes) do not prevent future runs.
    """
    _ensure_job_locks()
    con = _db_connect()
    try:
        now = int(time.time() * 1000)
        con.execute(
            "DELETE FROM job_locks WHERE expires_ms<=?",
            (now,),
        )
        con.commit()
    finally:
...
@contextmanager
def job_lock(name: str, ttl_ms: int = 10_000):
    """Context manager that acquires a job lock and releases it on exit.

    Raises RuntimeError if lock cannot be acquired (another instance running).
    """
    acquired = acquire_lock(name, ttl_ms)
    if not acquired:
