            ...
            try:
                ...
                if int(realized) == 1:
                    try:
                        ...
                        # execution noise check
                        try:
                            noise = float(cost.get("slippage_bps") or 0.0) + float(cost.get("fees_bps") or 0.0)
                            if noise > float(os.environ.get("EXEC_NOISE_RETRAIN_THRESHOLD", "0.005")):
                                from engine.strategy.training_hooks import schedule_retraining
                                schedule_retraining(model_name=str(model_name), reason="exec_noise")
                        except Exception:
                            pass
                    except Exception:
                        pass
