...
W_SELF_CRITIC = float(os.environ.get("SHADOW_W_SELF_CRITIC", "1.0"))
...
def _read_shadow_metrics(con, since_ms: int, regime: str) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        # Latest metrics per model_name in window. Regime is best-effort:
        # if a regime column exists, prefer it; otherwise ignore.
        cols = [r[1] for r in con.execute("PRAGMA table_info(shadow_metrics)").fetchall() or []]
        has_regime = "regime" in set(str(c or "") for c in cols)

        if has_regime:
            rows = con.execute(
                """
                SELECT model_name, rmse, dir_acc, net_rmse, n, ts_ms
                FROM shadow_metrics
                WHERE ts_ms >= ?
                  AND regime = ?
                ORDER BY ts_ms DESC
    ...
    # keep most recent per model_name
    seen = set()
    for model_name, rmse, dir_acc, net_rmse, n, ts_ms in rows or []:
        mn = str(model_name or "").strip()
        if not mn or mn in seen:
            continue
        seen.add(mn)
        out[mn] = {
            "rmse": _safe_float(rmse, None),
            "dir_acc": _safe_float(dir_acc, None),
            "net_rmse": _safe_float(net_rmse, None),
            "n": int(n or 0),
...
def _read_self_critic_counts_by_model(con, since_ms: int) -> Dict[str, Dict[str, Any]]:
    """
    Best-effort: read Self-Critic warnings counts per model_name.
    Returns:
       model_name -> {crit, warn}
    """
    out: Dict[str, Dict[str, Any]] = {}
    if not _table_exists(con, "self_critic_warnings"):
        return out
    try:
        rows = con.execute(
            """
            SELECT model_name, severity, COUNT(1)
            FROM self_critic_warnings
            WHERE ts_ms >= ?
               AND model_name IS NOT NULL
               AND severity IN ('CRIT','WARN')
            GROUP BY model_name, severity
            """,
            (int(since_ms),),
        ).fetchall()
    except Exception:
    ...
    for name, sev, n in rows or []:
        mn = str(name or "").strip()
        if not mn:
            continue
        s = str(sev or "").upper().strip()
        out.setdefault(mn, {"crit": 0, "warn": 0})
        if s == "CRIT":
            out[mn]["crit"] = int(n or 0)
        elif s == "WARN":
            out[mn]["warn"] = int(n or 0)
    return out
...
def compute_and_persist_shadow_capital_scores(
    *,
    ...
    try:
        ...
        sc = _read_self_critic_counts_by_model(con, since_ms=since_ms)
        ...
        for model_name, m in (sm or {}).items():
            ...
            sc_crit = int((sc.get(model_name) or {}).get("crit") or 0)
            sc_warn = int((sc.get(model_name) or {}).get("warn") or 0)
            sc_penalty = float((2 * sc_crit) + (1 * sc_warn))
            ...
            score = (
                ...
                - float(W_SELF_CRITIC) * float(sc_penalty)
            )
            ...
            components = {
                ...
                "cap_eff": cap_eff,
                "self_critic_crit": sc_crit,
                "self_critic_warn": sc_warn,
                "self_critic_penalty": sc_penalty,
            }
