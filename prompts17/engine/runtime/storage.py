...
def _new_connection(*, readonly: bool) -> sqlite3.Connection:
    ...
    if _INTEGRITY_CHECK_ON_START and not readonly:
        try:
            ...
            # attempt to recover by moving corrupt database aside and returning a fresh one
            try:
                path = DB_PATH
                ...
                pass
            # recursively try again once, without integrity check to avoid infinite loop
            if not readonly:
                try:
                    return _new_connection(readonly=readonly)
                except Exception:
                    pass
            raise
...
def _ensure_labels_columns(con):
    ...
    if not _has_column(con, "labels", "label_meta_json"):
        con.execute("ALTER TABLE labels ADD COLUMN label_meta_json TEXT;")
...
def _ensure_model_marketplace_schema(con):
    # Additive, idempotent: Universe Selection + Model Marketplace layer
    # Deterministic, production-safe tables to:
    # - score realized net profitability per (symbol, horizon, model)
    # - decide eligibility/tier per model
    # - persist capital allocations + training budget plans
    con.executescript(
        """
        CREATE TABLE IF NOT EXISTS marketplace_horizons (
        ...
        CREATE INDEX IF NOT EXISTS idx_marketplace_horizons_active
          ON marketplace_horizons(is_active);

        CREATE TABLE IF NOT EXISTS marketplace_model_score (
          ts_ms INTEGER NOT NULL,
          ...
          PRIMARY KEY (ts_ms, window_days, symbol, horizon_s, model_name)
        );

        CREATE INDEX IF NOT EXISTS idx_marketplace_model_score_lookup
          ON marketplace_model_score(ts_ms, window_days, score);

        CREATE INDEX IF NOT EXISTS idx_marketplace_model_score_symbol
          ON marketplace_model_score(symbol, horizon_s, ts_ms);

        CREATE TABLE IF NOT EXISTS marketplace_model_state (
          symbol TEXT NOT NULL,
          horizon_s INTEGER NOT NULL,
          model_name TEXT NOT NULL,

          tier TEXT NOT NULL,               -- paper|candidate|live|blocked|retired
          eligible INTEGER NOT NULL,
          reason_blocked TEXT,
          promoted_ts_ms INTEGER,
          demoted_ts_ms INTEGER,
          last_decision_ts_ms INTEGER NOT NULL,

          PRIMARY KEY (symbol, horizon_s, model_name)
        );

        CREATE INDEX IF NOT EXISTS idx_marketplace_model_state_tier
          ON marketplace_model_state(tier, eligible);

        CREATE TABLE IF NOT EXISTS marketplace_symbol_score (
          ts_ms INTEGER NOT NULL,
          window_days INTEGER NOT NULL,
          symbol TEXT NOT NULL,
          ...
          stability REAL NOT NULL,
          score REAL NOT NULL,
          best_horizon_s INTEGER,
          best_model_name TEXT,
          detail_json TEXT,

          PRIMARY KEY (ts_ms, window_days, symbol)
        );

        CREATE INDEX IF NOT EXISTS idx_marketplace_symbol_score_rank
          ON marketplace_symbol_score(ts_ms, window_days, score);

        CREATE TABLE IF NOT EXISTS marketplace_allocation (
          ts_ms INTEGER NOT NULL,
          symbol TEXT NOT NULL,
          horizon_s INTEGER NOT NULL,
          model_name TEXT NOT NULL,
          ...
          reason_json TEXT,
          PRIMARY KEY (ts_ms, symbol, horizon_s, model_name)
        );

        CREATE INDEX IF NOT EXISTS idx_marketplace_allocation_ts
          ON marketplace_allocation(ts_ms);

        CREATE TABLE IF NOT EXISTS marketplace_training_budget (
          ts_ms INTEGER NOT NULL,
          scope TEXT NOT NULL,
          total_budget_units REAL NOT NULL,
          PRIMARY KEY (ts_ms, scope)
        );

        CREATE TABLE IF NOT EXISTS marketplace_training_plan (
          ts_ms INTEGER NOT NULL,
          symbol TEXT NOT NULL,
          horizon_s INTEGER NOT NULL,
          model_name TEXT NOT NULL,
          budget_units REAL NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS idx_marketplace_training_plan_ts_prio
          ON marketplace_training_plan(ts_ms, priority);
        """
    )
...
def _ensure_news_schema(con):
    # tables used by the news intelligence layer
    con.executescript(
        """
        ...
        CREATE INDEX IF NOT EXISTS idx_news_features_sym_bucket
          ON news_features(symbol, bucket_ts_ms);

        CREATE INDEX IF NOT EXISTS idx_news_features_bucket
          ON news_features(bucket_ts_ms);

        CREATE TABLE IF NOT EXISTS news_symbol_embeddings (
        ...
        CREATE INDEX IF NOT EXISTS idx_news_symemb_event
          ON news_symbol_embeddings(event_id);
        """
    )

    # -- new source performance / blacklist schemas --
    try:
        con.executescript(
            """
            CREATE TABLE IF NOT EXISTS source_perf (
              ts_ms INTEGER NOT NULL,
              ...
              PRIMARY KEY(source, symbol, regime, horizon_s)
            );

            CREATE INDEX IF NOT EXISTS idx_source_perf_source
              ON source_perf(source);

            CREATE INDEX IF NOT EXISTS idx_source_perf_symbol
              ON source_perf(symbol);
            ...
            CREATE INDEX IF NOT EXISTS idx_source_blacklist_symbol
              ON source_blacklist(symbol);

            CREATE INDEX IF NOT EXISTS idx_source_blacklist_updated
              ON source_blacklist(updated_ts_ms);
            """
        )
    except Exception:
        pass

    # Additive, idempotent migrations for provenance/audit
    try:
        if not _has_column(con, "news_symbol_embeddings", "method"):
            con.execute("ALTER TABLE news_symbol_embeddings ADD COLUMN method TEXT;")
    except Exception:
        pass

    try:
        if not _has_column(con, "news_symbol_embeddings", "created_ts_ms"):
            con.execute("ALTER TABLE news_symbol_embeddings ADD COLUMN created_ts_ms INTEGER;")
    except Exception:
        pass
...
def init_db():
    ...
    try:
        ...
        # news intelligence schema (buckets, symbol embeddings)
        _ensure_news_schema(con)
        ...
        _ensure_model_marketplace_schema(con)
