# dev_core/regime_compat.py
"""
Model × regime compatibility scoring with:
- exponential decay (trade-driven)
- staleness guard
- optional suppression gate

Safe-by-default:
- no effect if insufficient data
- no effect if stale
- suppression only after min trades AND not stale
"""
...
def regime_compat_multiplier(*, model_name: str, anchor: str = "SPY") -> Dict[str, Any]:
    ...
    try:
        ...
        # staleness guard => neutral (no impact)
        stale = False
        if MAX_STALE_DAYS > 0:
            age_ms = _now_ms() - last_ms
            if age_ms > int(MAX_STALE_DAYS * 86400000.0):
                stale = True
                # trigger retrain if regime compatibility stale
                try:
                    from engine.strategy.training_hooks import schedule_retraining
                    schedule_retraining(model_name=os.environ.get("MODEL_NAME", "embed_regressor"), reason="regime_scores_stale")
                except Exception:
                    pass
                return {
                    "mult": 1.0,
