# dev_core/corr_opt.py
"""
Correlation-aware convex optimizer for portfolio targets.

- Convex quadratic risk term: 0.5 * w^T Σ w
- Linear utility term:       -gamma * u^T |w|
- Bounded: 0 <= |w_i| <= cap_i
- Gross cap: sum_i |w_i| = gross_cap (if feasible; else <=)
- Sign is fixed by desired[sym]["side"] (LONG=+1, SHORT=-1)

Deterministic: no randomness, stable projections.
No Kelly.
"""
...
def _utility_from_target(tgt: Dict[str, Any]) -> float:
    ...
    else fallback to expected_ret_net / expected_dd from explain_json.
    """
    try:
        r = tgt.get("reason") or {}
...
def _build_covariance(con, syms: List[str], lookback: int) -> Tuple[List[List[float]], List[float]]:
    ...
    vols = []
    for s in syms:
        v = realized_vol_from_prices(con, s, lookback=int(lookback), ts_ms=now_ms)
        vols.append(_safe_float(v, 0.0))

    # floor vols to avoid singularities
    ...
    for i in range(n):
        for j in range(i + 1, n):
            c = corr_from_prices(con, syms[i], syms[j], lookback=int(lookback), ts_ms=now_ms)
            if c is None:
                cc = 0.0
            else:
