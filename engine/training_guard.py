...
CREATE INDEX IF NOT EXISTS idx_risk_state_ts ON risk_state(ts_ms);
"""
...
def _get_state_row(key: str, default: str) -> tuple[str, int]:
    ...
    con = _connect()
    try:
        r = con.execute("SELECT v, ts_ms FROM risk_state WHERE k=?", (str(key),)).fetchone()
        if not r:
...
def _set_state(key: str, value: str) -> None:
    ...
    try:
        con.execute(
            "INSERT OR REPLACE INTO risk_state(k, v, ts_ms) VALUES (?,?,?)",
