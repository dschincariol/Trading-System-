# dev_core/rules_engine.py
"""
Rules engine:
- Global stops: drawdown, max drift, global exec degradation
- Per-symbol halts: symbol exec degradation, symbol drift

Uses dev_core.kill_switch.activate/clear.
"""
...
from engine.dev_core.kill_switch import activate, clear
from engine.dev_core.drawdown_state import get_current_drawdown
from engine.dev_core.exec_stats import get_exec_winrate_global, get_exec_stats_by_symbol
from engine.dev_core.drift_utils import get_max_drift_ratio, get_symbol_max_drift_ratio
...
def _detect_realized_exec_cost_spike(con) -> Dict[str, Any]:
    """
    Uses realized execution costs from labels_exec.
    Triggers when pctl(total_cost_bps) exceeds threshold.
    """
    now_ms = _now_ms()
