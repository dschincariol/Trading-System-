"""
jobs/calibrate_confidence_from_prices.py

Learn confidence calibration curves from price-derived outcomes.

We train per (symbol, horizon_s) a monotonic-ish mapping from raw model confidence
to empirical win-rate using simple binning.

Inputs:
- labels_price (ret, dir) derived from prices
- decisions (predicted_z, confidence) OR validation table if you store confidence there

Output:
- confidence_calibration rows with payload_json describing bins and win rates

This is intentionally simple, stable, and production-safe.
"""
...
def _calibrate_for_symbol(con, symbol: str, horizon_s: int, since_ms: int) -> bool:
    """
    Join decisions to labels_price to build (confidence, win) samples.
    Assumes decisions table exists with ts_ms, symbol, horizon_s, predicted_z, confidence.
    """
    rows = con.execute(
        """
        SELECT d.confidence, d.predicted_z, lp.dir
        FROM decisions d
        JOIN labels_price lp
          ON lp.symbol = d.symbol
         AND lp.horizon_s = d.horizon_s
         AND lp.ts_pred_ms = d.ts_ms
        WHERE d.symbol = ?
          AND d.horizon_s = ?
          AND d.ts_ms >= ?
...
def main():
    ...
    try:
        ...
        try:
            ...
            if not symbols:
                logging.warning("no active symbols found; nothing to calibrate")
                return
            ...
            for sym in symbols:
                ...
                for h in (300, 3600):
                    ...
                    try:
                        ...
                        logging.warning("calibration failed sym=%s h=%s err=%r", sym, h, e)
