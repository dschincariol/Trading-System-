...
"""
Regime-adaptive capital scaling (institutional-grade, deterministic).

Purpose:
  Provide a single regime scaling factor that can safely compress or expand
  gross exposure based on:
    - Regime (LOW/MID/HIGH) derived from anchor realized vol
    - Regime confidence (distance from regime thresholds)
    - VIX stress z-score (optional)
    - Rolling portfolio drawdown (optional)

This is applied in dev_core.portfolio.compute_rebalance() after other sizing
policies, then re-normalized to PORTFOLIO_GROSS_CAP.

Env (defaults conservative):
  PORTFOLIO_REGIME_SCALE_ENABLE=1
  PORTFOLIO_REGIME_ANCHOR=SPY
  ...
  # Confidence scaling (near threshold => compress toward floor)
  PORTFOLIO_REGIME_CONF_ENABLE=1
  PORTFOLIO_REGIME_CONF_FLOOR=0.65
  PORTFOLIO_REGIME_CONF_BAND=0.002  # vol distance to boundary for "full confidence"

  # VIX stress scaling (optional)
  PORTFOLIO_REGIME_VIX_ENABLE=1
  PORTFOLIO_REGIME_VIX_Z_TH=2.0
  PORTFOLIO_REGIME_VIX_Z_AT_MIN=4.0
  PORTFOLIO_REGIME_VIX_MIN=0.60

  # Drawdown scaling (optional)
  PORTFOLIO_REGIME_DD_ENABLE=1
  PORTFOLIO_REGIME_DD_LOOKBACK_D=14
  PORTFOLIO_REGIME_DD_TH=0.06
  PORTFOLIO_REGIME_DD_AT_MIN=0.15
  PORTFOLIO_REGIME_DD_MIN=0.55
"""
...
from engine.dev_core.model_v2 import get_current_regime, classify_regime
...
def _anchor_realized_vol(con, symbol: str, lookback: int = 120) -> Optional[float]:
    try:
        ...
        if len(rets) < 25:
            return None
        # RMS return proxy (aligned with model_v2.get_current_regime)
