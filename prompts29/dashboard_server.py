...
# -------------            -- ------------------------------------------------------
UI_DEMO_MODE = os.environ.get("UI_DEMO_MODE", "0") == "1"
UI_DEMO_SCENARIO = os.environ.get("UI_DEMO_SCENARIO", "default")

# Demo mode state (in-memory only, resets on restart)
_demo_state = {
    "enabled": UI_DEMO_MODE,
    "scenario": UI_DEMO_SCENARIO,
    "started": False,
    "start_time_ms": None,
    "current_step": 0,
    "total_steps": 0,
...
DEMO_SCENARIOS = {
    "default": {
        "name": "Default Market Scenario",
        "description": "Balanced market with moderate volatility",
        "duration_minutes": 5,
        "events": [
            {"step": 0, "type": "INGEST", "label": "Market Data Ingest", "description": "GDELT news ingestion completed", "delay_ms": 0},
            {"step": 1, "type": "MODEL", "label": "Model Training", "description": "Random Forest model updated", "delay_ms": 2000},
            {"step": 2, "type": "RISK", "label": "Risk Alert", "description": "AAPL volatility spike detected", "delay_ms": 4000},
            {"step": 3, "type": "DECISION", "label": "Portfolio Decision", "description": "Reduce AAPL position by 2.5%", "delay_ms": 6000},
            {"step": 4, "type": "EXECUTION", "label": "Order Execution", "description": "Sell order executed for AAPL", "delay_ms": 8000},
        ]
    },
    "high_volatility": {
        "name": "High Volatility Scenario",
        "description": "Market stress with multiple risk events",
        "duration_minutes": 3,
        "events": [
            {"step": 0, "type": "INGEST", "label": "RSS Feed Ingest", "description": "Financial news RSS processed", "delay_ms": 0},
            {"step": 1, "type": "RISK", "label": "Critical Alert", "description": "Market stress threshold exceeded", "delay_ms": 1000},
            {"step": 2, "type": "MODEL", "label": "Model Update", "description": "LSTM model recalibrated", "delay_ms": 2000},
            {"step": 3, "type": "RISK", "label": "Risk Warning", "description": "TSLA price movement anomaly", "delay_ms": 3000},
            {"step": 4, "type": "DECISION", "label": "Risk Decision", "description": "Reduce exposure across portfolio", "delay_ms": 4000},
            {"step": 5, "type": "EXECUTION", "label": "Batch Execution", "description": "Multiple positions adjusted", "delay_ms": 5000},
        ]
    },
    "steady_state": {
        "name": "Steady State Scenario",
        "description": "Normal market conditions with routine operations",
        "duration_minutes": 4,
        "events": [
            {"step": 0, "type": "INGEST", "label": "Price Data", "description": "End-of-day price ingestion", "delay_ms": 0},
            {"step": 1, "type": "MODEL", "label": "Model Validation", "description": "Model performance check passed", "delay_ms": 3000},
            {"step": 2, "type": "INGEST", "label": "Options Data", "description": "Options chain processed", "delay_ms": 6000},
            {"step": 3, "type": "DECISION", "label": "Rebalance", "description": "Minor portfolio rebalance", "delay_ms": 9000},
        ]
    }
}

def _get_demo_scenario(scenario_id):
    """Get demo scenario by ID, fallback to default if not found"""
    return DEMO_SCENARIOS.get(scenario_id, DEMO_SCENARIOS["default"])

def _generate_demo_timeline_event(scenario_event, base_time_ms):
    """Generate a synthetic timeline event from scenario data"""
    return {
        "id": f"demo_{scenario_event['type'].lower()}_{scenario_event['step']}",
        "ts_ms": base_time_ms + scenario_event['delay_ms'],
        "type": scenario_event['type'],
        "label": scenario_event['label'],
        "description": scenario_event['description'],
        "reference_id": f"demo_ref_{scenario_event['step']}",
        "demo": True
    }

def _generate_demo_decision(base_time_ms):
    """Generate a synthetic decision event"""
    import random
    ...
    size_delta = random.uniform(0.01, 0.15)
    certainty = random.uniform(0.6, 0.95)
    
    return {
        "decision_id": f"demo_decision_{int(base_time_ms)}",
        "ts_ms": base_time_ms,
        "action": action,
        "symbol": symbol,
        "size_delta": size_delta,
        "certainty": certainty,
        "risk_impact": "high" if size_delta > 0.1 else "medium",
        "why": f"Demo: {action} {symbol} position by {size_delta:.2%} based on synthetic analysis",
        "from_weight": random.uniform(0.05, 0.25),
        "to_weight": random.uniform(0.05, 0.25),
        "source_alert_id": f"demo_alert_{int(base_time_ms)}",
        "demo": True
    }
...
def api_get_ui_overview(_parsed, _ctx=None):
    ...
    # Inject demo data if active
    return _inject_demo_data(response, "overview")
...
def api_get_decisions(parsed):
    ...
    try:
        ...
        try:
            ...
            # Inject demo data if active
            return _inject_demo_data(response, "decisions")
...
def api_get_timeline(parsed):
    ...
    try:
        ...
        try:
            ...
            # Inject demo data if active
            return _inject_demo_data(response, "timeline")
...
def api_get_graph_snapshot(parsed):
    """
    GET /api/ui/graph_snapshot
    Returns nodes and edges representing the live system architecture:
    ingestion → features → models → risk gates → portfolio → execution
    Includes module health (last heartbeat / last error)
    """
    import time
    from engine.runtime.storage import connect as _db_connect
    ...
    try:
        ...
        try:
            ...
            stages = [
                {"id": "ingestion", "label": "Data Ingestion", "type": "ingestion"},
                {"id": "features", "label": "Feature Engineering", "type": "features"},
                {"id": "models", "label": "ML Models", "type": "models"},
                {"id": "risk_gates", "label": "Risk Gates", "type": "risk"},
                {"id": "portfolio", "label": "Portfolio Management", "type": "portfolio"},
                {"id": "execution", "label": "Trade Execution", "type": "execution"}
            ]
            
            # Create edges (data flow)
            ...
            # Get health information for each stage
            for stage in stages:
                stage_id = stage["id"]
                ...
                if stage_id == "ingestion":
                    ...
                    try:
                        rows = con.execute("""
                            SELECT ts_ms, event
                            FROM job_history 
                            WHERE job_name LIKE '%ingest%' OR job_name LIKE '%poll%'
                            ORDER BY ts_ms DESC 
                            LIMIT 10
                        """).fetchall()
                        
                        if rows:
                            last_heartbeat = rows[0]['ts_ms']
                            activity_count = len(rows)
                            
                            # Check for recent errors
                            error_rows = [r for r in rows if 'error' in str(r['event']).lower()]
                            if error_rows:
                                health_status = "degraded"
                                last_error = error_rows[0]['event']
                    except Exception:
                    ...
                    try:
                        rows = con.execute("""
                            SELECT ts_ms
                            FROM job_history 
                            WHERE job_name LIKE '%feature%' OR event LIKE '%feature%'
                            ORDER BY ts_ms DESC 
                            LIMIT 10
                        """).fetchall()
                ...
                elif stage_id == "models":
                    # Check model registry and training
                    try:
                        rows = con.execute("""
                            SELECT model_ts_ms, stage
                            FROM model_registry
                        ...
                        if rows:
                            last_heartbeat = rows[0]['model_ts_ms']
                            activity_count = len(rows)
                            
                            # Check if models are stuck in training
                            training_count = sum(1 for r in rows if r['stage'] == 'training')
                            if training_count > 3:
                                health_status = "degraded"
                    except Exception:
                        health_status = "unknown"
                
                elif stage_id == "risk_gates":
                    # Check recent alerts and risk checks
                    try:
                        rows = con.execute("""
                            SELECT ts_ms, severity
                            FROM alerts
                            WHERE ts_ms > ?
                            ORDER BY ts_ms DESC
                            LIMIT 20
                        """, (now_ms - 3600000,)).fetchall()  # Last hour
                        ...
                        if rows:
                            ...
                            elif sum(1 for r in rows if r['severity'] == 'WARNING') > 5:
                                health_status = "degraded"
                        else:
                            health_status = "inactive"
                    except Exception:
                    ...
                    # Check portfolio orders and state
                    try:
                        rows = con.execute("""
                            SELECT ts_ms
                            FROM portfolio_orders
                    ...
                    # Check execution metrics and broker fills
                    try:
                        rows = con.execute("""
                            SELECT ts_ms
                        ...
                        if rows:
                            ...
                            # Check for recent execution attempts
                            exec_rows = con.execute("""
                                SELECT ts_ms
                                FROM job_history 
                                WHERE job_name LIKE '%execution%' OR event LIKE '%execution%'
                                ORDER BY ts_ms DESC
                                LIMIT 5
                            """).fetchall()
                            
                            if exec_rows:
                                last_heartbeat = exec_rows[0]['ts_ms']
                                activity_count = len(exec_rows)
                                health_status = "degraded"  # No fills but attempts
                            else:
                                health_status = "inactive"
                    except Exception:
                        health_status = "unknown"
                ...
                heartbeat_age_s = (now_ms - last_heartbeat) // 1000
                
                # Determine health based on heartbeat age
                if heartbeat_age_s > 300:  # 5 minutes
                    if health_status == "healthy":
                        health_status = "stale"
                elif heartbeat_age_s > 900:  # 15 minutes
                    health_status = "inactive"
        ...
        log.error("api_get_graph_snapshot failed: %s", e)
        return {
            "ok": False,
            "error": str(e),
            "nodes": [],
...
# -------------            -- ------------------------------------------------------
def api_get_demo_state(parsed):
    """
    GET /api/ui/demo/state
    Returns current demo mode state and available scenarios
    """
    with _demo_lock:
        return {
            "ok": True,
            "demo_enabled": _demo_state["enabled"],
            "demo_started": _demo_state["started"],
            "current_scenario": _demo_state["scenario"],
            "current_step": _demo_state["current_step"],
            "total_steps": _demo_state["total_steps"],
            "start_time_ms": _demo_state["start_time_ms"],
            "available_scenarios": {
                sid: {
                    "name": scenario["name"],
...
def api_post_demo_start(parsed):
    """
    POST /api/ui/demo/start
    Starts demo mode with specified scenario
    Body: {"scenario": "scenario_id"}
    """
    if not _demo_state["enabled"]:
        return {"ok": False, "error": "Demo mode not enabled"}
    
    scenario_id = parsed.get("scenario", UI_DEMO_SCENARIO)
    scenario = _get_demo_scenario(scenario_id)
    
    with _demo_lock:
        _demo_state["started"] = True
        _demo_state["scenario"] = scenario_id
        _demo_state["start_time_ms"] = int(time.time() * 1000)
        _demo_state["current_step"] = 0
        _demo_state["total_steps"] = len(scenario["events"])
        _demo_state["events"] = []
        
        # Pre-generate all timeline events
        for event in scenario["events"]:
            timeline_event = _generate_demo_timeline_event(event, _demo_state["start_time_ms"])
            _demo_state["events"].append(timeline_event)
        
        # Generate some demo decisions
        for i in range(3):
            decision_time = _demo_state["start_time_ms"] + (i * 3000) + 1500
            decision = _generate_demo_decision(decision_time)
            _demo_state["events"].append(decision)
    
    log.info("Demo started: scenario=%s, events=%d", scenario_id, len(_demo_state["events"]))
    ...
    return {
        ...
        "duration_minutes": scenario["duration_minutes"],
        "events_generated": len(_demo_state["events"]),
        "start_time_ms": _demo_state["start_time_ms"]
    }

def _is_demo_active():
    """Check if demo mode is active and should inject synthetic data"""
    if not _demo_state["enabled"] or not _demo_state["started"]:
        return False
    
    # Check if demo should still be running (based on scenario duration)
    scenario = _get_demo_scenario(_demo_state["scenario"])
    elapsed_ms = int(time.time() * 1000) - _demo_state["start_time_ms"]
    duration_ms = scenario["duration_minutes"] * 60 * 1000
    
    return elapsed_ms < duration_ms

def _inject_demo_data(original_response, endpoint_type):
    """Inject demo data into API responses if demo mode is active"""
    if not _is_demo_active():
        return original_response
    
    # Only modify successful responses
    if not original_response.get("ok"):
        return original_response
    ...
    with _demo_lock:
        ...
        if endpoint_type == "timeline":
            # Add demo events to timeline
            demo_events = [
                event for event in _demo_state["events"]
                if event["ts_ms"] <= current_ms and "type" in event
            ]
            
            if "entries" not in original_response:
                original_response["entries"] = []
            
            # Merge demo events with real events, demo events first
            demo_entries = [e for e in demo_events if e.get("demo")]
            real_entries = [e for e in original_response["entries"] if not e.get("demo")]
            
            original_response["entries"] = demo_entries + real_entries[:50]  # Limit real events
            original_response["count"] = len(original_response["entries"])
            original_response["demo_active"] = True
            
        elif endpoint_type == "decisions":
            # Add demo decisions
            demo_decisions = [
                event for event in _demo_state["events"]
                if event["ts_ms"] <= current_ms and "action" in event
            ]
            
            if "decisions" not in original_response:
                original_response["decisions"] = []
            
            # Merge demo decisions with real decisions
            demo_decision_list = [d for d in demo_decisions if d.get("demo")]
            real_decisions = [d for d in original_response["decisions"] if not d.get("demo")]
            
            original_response["decisions"] = demo_decision_list + real_decisions[:20]  # Limit real decisions
            original_response["count"] = len(original_response["decisions"])
            original_response["demo_active"] = True
            
        elif endpoint_type == "overview":
            # Add demo indicators to overview
            original_response["demo_active"] = True
            original_response["demo_scenario"] = _demo_state["scenario"]
            original_response["system_health"] = "healthy"  # Demo always shows healthy
...
# Architecture UI route specs
ROUTE_SPECS_ARCHITECTURE = [
    ("GET", "/api/ui/graph_snapshot", "api_get_graph_snapshot"),
]

# Demo UI route specs
ROUTE_SPECS_DEMO = [
    ("GET", "/api/ui/demo/state", "api_get_demo_state"),
    ("POST", "/api/ui/demo/start", "api_post_demo_start"),
]
...
ROUTE_SPECS = (
    ...
    + list(ROUTE_SPECS_ARCHITECTURE)
    + list(ROUTE_SPECS_DEMO)
)
...
API_HANDLERS = {
    ...
    "api_get_timeline": api_get_timeline,
    "api_get_graph_snapshot": api_get_graph_snapshot,
    ...
    # DEMO MODE
    "api_get_demo_state": api_get_demo_state,
    "api_post_demo_start": api_post_demo_start,
}
