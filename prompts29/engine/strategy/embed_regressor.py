...
def train_embed_models(
    ...
    # optionally drop very old events to avoid stale embeddings
    MAX_EVENT_AGE_DAYS = float(os.environ.get("EMB_TRAIN_MAX_EVENT_AGE_DAYS", "730"))
    max_age_ms = int(MAX_EVENT_AGE_DAYS * 86400 * 1000)
    ...
    try:
        ...
        for _eid, sym, h, z, blob, ts_ms in rows or []:
            ...
            ts_i = int(ts_ms or 0)
            # skip if event older than MAX_EVENT_AGE_DAYS
            if max_age_ms > 0 and (now_ms - ts_i) > max_age_ms:
                continue
