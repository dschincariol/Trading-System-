...
async function checkDemoState() {
  try {
    const response = await fetchJSON('/api/ui/demo/state');
    if (response && response.ok) {
      demoMode.enabled = response.demo_enabled;
      demoMode.started = response.demo_started;
      demoMode.scenario = response.current_scenario;
      
      // Update demo badge visibility
      const demoBadge = document.getElementById('demoBadge');
      if (demoBadge) {
        demoBadge.style.display = demoMode.enabled ? 'inline-flex' : 'none';
      }
      
      // Auto-start demo if enabled but not started
      if (demoMode.enabled && !demoMode.started && !demoMode.autoStartAttempted) {
        demoMode.autoStartAttempted = true;
        await startDemoMode(demoMode.scenario);
      }
    }
  } catch (error) {
    console.warn('Failed to check demo state:', error);
  }
}

async function startDemoMode(scenario = 'default') {
  try {
    const response = await postJSON('/api/ui/demo/start', { scenario });
    if (response && response.ok) {
      demoMode.started = true;
      demoMode.scenario = scenario;
      console.log(`Demo started: ${response.scenario_name}`);
      
      // Trigger refresh of all demo-aware components
      setTimeout(() => {
        loadExecutiveOverview();
        loadDecisions();
        if (window.timelineUI) {
          window.timelineUI.loadTimeline();
        }
      }, 500);
    }
  } catch (error) {
...
async function openIncidentDrawer(row) {
  ...
  if (facts) {
    ...
    facts.innerHTML = `
      ...
      <div class="kvK">certainty</div><div class="kvV">${Number.isFinite(conf) ? conf.toFixed(2) : "—"}</div>
...
async function openWhyModal(alertRow) {
  ...
  out += `certainty: ${Number(alertRow.confidence).toFixed(2)}\n`;
  if (typeof ex.confidence_base === "number") {
    out += `certainty_base: ${Number(ex.confidence_base).toFixed(2)}\n`;
  }
  ...
  if (ex.rule) {
    ...
    out += `min_cert=${Number(ex.rule.min_conf || 0).toFixed(2)}\n`;
...
renderIncidentQueue(
  ...
  for (const r of _lastAlerts) {
    ...
    const decisionConf =
      (conf >= 0.85 && z >= 1.5) ? "High certainty decision" :
      (conf >= 0.65) ? "Moderate certainty decision" :
      "Low certainty decision";
  ...
  try {
    ...
    if (Number(s.z_rates) > 1.2) reasons.push("Rates risk-off signal");
      ...
      hdr.title = reasons.length
        ? `Market Stress drivers: ${reasons.join(", ")}`
    ...
    const cert = document.getElementById("certaintyBadge");
    if (cert) {
      ...
      cert.className = cls;
      cert.textContent = `${Math.round(c * 100)}%`;
      cert.title = "Displayed certainty dampening due to market stress (informational only)";
    }
...
const presentationMode = {
  ...
  enable() {
    ...
    const text = document.getElementById('presentationModeText');
    
    if (toggle) toggle.style.background = '#58a6ff';
    if (toggle) toggle.style.color = 'white';
    if (icon) icon.textContent = '🎯';
    if (text) text.textContent = 'Presentation: ON';
    
    // Hide technical elements
    this.hideTechnicalElements();
    
    // Enlarge key widgets
    this.enlargeKeyWidgets();
    
    // Save state
    localStorage.setItem('presentationMode', 'true');
    
    toast('Presentation Mode enabled - Technical details hidden', 'ok', 2000);
  },
  ...
  disable() {
    ...
    if (toggle) toggle.style.background = '#21262d';
    if (toggle) toggle.style.color = '#9da7b3';
    if (icon) icon.textContent = '🎯';
    if (text) text.textContent = 'Presentation: OFF';
    
    // Show hidden elements
    this.showTechnicalElements();
    ...
    toast('Presentation Mode disabled', 'ok', 2000);
  },
  
  hideTechnicalElements() {
    // Hide elements with technical content
    ...
    technicalSelectors.forEach(selector => {
      document.querySelectorAll(selector).forEach(el => {
        el.classList.add('presentation-hidden');
      });
    });
  },
  
  showTechnicalElements() {
    document.querySelectorAll('.presentation-hidden').forEach(el => {
      el.classList.remove('presentation-hidden');
    });
  },
  
  enlargeKeyWidgets() {
    // Enlarge important widgets for presentation
    const keySelectors = [
      '[id*="health"]',
      '[id*="market"]', 
      '[id*="executive"]',
    ...
    keySelectors.forEach(selector => {
      document.querySelectorAll(selector).forEach(el => {
        el.classList.add('presentation-enlarged');
      });
    });
  },
  
  resetWidgetSizes() {
    document.querySelectorAll('.presentation-enlarged').forEach(el => {
      el.classList.remove('presentation-enlarged');
    });
  }
};

/* -----------------------------
   Visual Consistency & Motion
----------------------------- */
function applyVisualConsistency() {
  // Standardize status colors across all pills
  standardizeStatusColors();
  
  // Add smooth transitions to progress bars
  enhanceProgressBars();
  
  // Add gentle highlights to new timeline events
  enhanceTimelineEvents();
}

function standardizeStatusColors() {
  ...
  document.querySelectorAll('.pill').forEach(pill => {
    ...
    for (const [status, className] of Object.entries(statusMappings)) {
...
function enhanceProgressBars() {
  // Add smooth transitions to all progress bar-like elements
  document.querySelectorAll('[style*="width"], [style*="height"]').forEach(element => {
    if (element.style.width || element.style.height) {
      element.classList.add('progress-bar', 'smooth');
    }
  });
  
  // Enhance certainty bars specifically
  document.querySelectorAll('.certainty-bar, [class*="confidence"]').forEach(bar => {
    bar.classList.add('certainty-bar');
    
    // Add updating animation during data refresh
    const observer = new MutationObserver(() => {
      bar.classList.add('updating');
      setTimeout(() => bar.classList.remove('updating'), 300);
    });
    observer.observe(bar, { attributes: true, attributeFilter: ['style'] });
  });
}

function enhanceTimelineEvents() {
  // Add highlight animation to new timeline entries
      ...
      mutations.forEach(mutation => {
        mutation.addedNodes.forEach(node => {
          if (node.nodeType === 1 && node.classList) {
            node.classList.add('timeline-event-new', 'card-fade-in');
          }
        });
      });
    });
  ...
  // Add gentle pulse to active/real-time elements
  document.querySelectorAll('.pill.ok, .status-healthy').forEach(element => {
    if (element.textContent.includes('live') || element.textContent.includes('active')) {
      element.classList.add('pulse-gentle');
    }
  });
}

// Apply visual enhancements after page load
setTimeout(applyVisualConsistency, 1000);
...
function setDefaultViews() {
  ...
  if (isDemoMode) {
    // Demo mode default: open Live Architecture tab first
    const architectureCard = document.querySelector('card:has(#architectureContainer)');
    if (architectureCard) {
      architectureCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  } else {
    // Live mode default: show Executive Overview first
    const execOverview = document.querySelector('.card:has(.appHeader) + .card');
    if (execOverview) {
      execOverview.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  }
  
  // Set default decision filters to "today" and "WARN+"
  setDefaultDecisionFilters();
}

function setDefaultDecisionFilters() {
  // Try to set decision filters if the UI supports filtering
  const dateFilter = document.getElementById('decisionDateFilter');
  const severityFilter = document.getElementById('decisionSeverityFilter');
  
  if (dateFilter) {
    // Set to "today" if available
    const todayOption = dateFilter.querySelector('option[value="today"]');
    if (todayOption) {
  ...
  if (severityFilter) {
    // Set to "WARN+" if available
    const warnOption = severityFilter.querySelector('option[value="WARN"]');
    if (warnOption) {
      severityFilter.value = 'WARN';
    }
  }
}
...
const guidedTour = {
  ...
  steps: [
    {
      target: '#telemetryStrip',
      title: 'System Health Bar',
      content: 'This telemetry strip shows key system metrics including NAV, returns, drawdown, and market stress levels. It\'s your at-a-glance view of overall system performance.',
      position: 'bottom'
    },
    {
      target: '#decisionsContainer',
      title: 'Recent Decisions',
      content: 'Here you\'ll see the latest trading decisions made by the system. Each card shows the action, certainty level, and rationale for the decision.',
      position: 'right'
    },
    {
      target: '#timelineContainer',
      title: 'Activity Timeline',
      content: 'The timeline provides a chronological view of all system events including data ingestion, strategy updates, and risk assessments.',
      position: 'left'
    },
    {
      target: '#architectureContainer',
      title: 'Live Architecture',
      content: 'This visualization shows the real-time system architecture and data flow between components. It helps you understand how the system is operating.',
      position: 'top'
    },
    {
      target: '.appHeader',
      title: 'Dashboard Controls',
      content: 'The header contains system status, demo mode indicators, and access to different views. You can toggle presentation mode here for investor meetings.',
      position: 'bottom'
    }
  ],
  ...
  init() {
    ...
    if (!localStorage.getItem('guidedTourCompleted')) {
  ...
  showStep(stepIndex) {
    ...
    if (!target || !highlight || !tooltip || !content) {
      this.skip();
      return;
    }
    ...
    highlight.classList.add('tour-highlight-active');
    ...
    tooltipTop = Math.max(16, Math.min(tooltipTop, window.innerHeight - tooltipRect.height - 16));
    ...
    nextBtn.textContent = stepIndex === this.steps.length - 1 ? 'Complete Tour' : 'Next →';
  ...
  complete() {
    ...
    if (overlay) overlay.style.display = 'none';
    if (highlight) highlight.classList.remove('tour-highlight-active');
    
    localStorage.setItem('guidedTourCompleted', 'true');
    toast('Tour completed! You can restart it anytime from the help menu.', 'ok', 3000);
  }
};
  ...
  try {
    ...
    if (data.demo_active) {
      ...
      if (demoBadge) {
        ...
        demoBadge.innerHTML = `🎭 DEMO: ${data.demo_scenario || 'active'}`;
      }
    }
    ...
    if (marketConf && data.market_environment) {
      marketConf.textContent = `Certainty: ${data.market_environment.confidence || "Unknown"}`;
    }
...
refresh().then(async () => {
  ...
  // Set default views based on mode
  setTimeout(setDefaultViews, 500);
  
  // Check and initialize demo mode
  checkDemoState();
  
  // Initialize Live Architecture
  initArchitecture();
...
async function loadDecisions() {
  ...
  try {
    ...
    // Show demo indicator if active
    if (response.demo_active) {
      const demoBadge = document.getElementById('demoBadge');
      if (demoBadge) {
        demoBadge.style.display = 'inline-flex';
        demoBadge.innerHTML = `🎭 DEMO: ${response.demo_scenario || 'active'}`;
      }
    }
...
function createDecisionCard(decision) {
  ...
  // Add demo indicator if this is a demo decision
  const demoIndicator = decision.demo ? '<span class="pill crit" style="animation: pulse 2s infinite;">🎭 DEMO</span>' : '';
  ...
  card.innerHTML = `
    ...
    <div class="incidentSub">
      ...
      <div style="display:flex; gap:12px; margin-top:4px;">
        <div>Certainty: ${(decision.certainty * 100).toFixed(0)}%</div>
    ...
    logsContainer.innerHTML = decision.decision_logs.map(log => `
      <div style="margin-bottom: 8px; padding: 8px; background: var(--panel2); border-radius: 6px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 11px;">
          ...
          <div>Certainty: ${(log.confidence * 100).toFixed(0)}%</div>
