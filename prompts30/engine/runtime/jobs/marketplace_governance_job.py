...
def _aggregate(con, *, window_days: int, asof_ms: int) -> Tuple[Dict[Tuple[str, int, str], Dict[str, Any]], Dict[str, Dict[str, Any]]]:
    ...
    # Optional: execution-cost aggregates from labels_exec (net-of-cost engine)
    exec_cost_by_sym_h: Dict[Tuple[str, int], Dict[str, float]] = {}
    try:
        chk = con.execute(
    ...
    if chk:
        try:
            ex_rows = con.execute(
                """
                SELECT symbol, horizon_s, COUNT(*) as n, AVG(total_cost_bps) as avg_cost_bps
                FROM labels_exec
                WHERE ts_ms >= ?
                  AND realized = 1
            ...
            for sym, h, n, avg_cost in ex_rows or []:
                s = str(sym or "").strip().upper()
                hh = int(h or 0)
                if not s:
                    continue
                exec_cost_by_sym_h[(s, hh)] = {
                    "n": float(n or 0.0),
                    "avg_cost_bps": float(avg_cost or 0.0),
                }
        except Exception:
            exec_cost_by_sym_h = {}
    ...
    for (sym, horizon_s, model_name), rec in by_key.items():
        ...
        # Prefer total execution costs if available, else fees-only.
        exec = exec_cost_by_sym_h.get((sym, int(horizon_s))) or {}
        avg_cost_bps = _safe_float(exec.get("avg_cost_bps"), 0.0)
        # Convert bps to a ratio comparable to pnl magnitude proxy (gross_abs is in $-like units).
        # This is a heuristic penalty term; scale via MP_MAX_COST_RATIO / W_COST.
        exec_cost_ratio = float((avg_cost_bps / 10000.0))
        fee_ratio = float((fees_sum / gross_abs) if gross_abs > 1e-12 else 0.0)
        cost_ratio = float(fee_ratio + exec_cost_ratio)
        ...
        model_out[(sym, horizon_s, model_name)] = {
            ...
            "fees": float(fees_sum),
            "avg_exec_cost_bps": float(avg_cost_bps),
