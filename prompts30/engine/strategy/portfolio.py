...
from engine.horizons import horizon_priority
...
# Horizon preference when multiple horizons compete for the same symbol.
# If enabled, score *= horizon_priority(horizon_s) which defaults to favor short horizons.
PORTFOLIO_USE_HORIZON_PRIORITY = os.environ.get("PORTFOLIO_USE_HORIZON_PRIORITY", "1") == "1"
...
def _load_strategy_capacity(con, lookback_days: int = 30) -> Dict[str, Dict[str, float]]:
    """Estimate strategy capacity limits from realized execution capital usage.

    Uses execution_capital_efficiency.strategy_name and notional.
    Capacity proxy: p80 of daily notional traded per strategy over lookback.

    Returns:
      {strategy_name: {"cap_notional_p80": float, "days": float}}

    Fail-soft: returns {} on any error or missing table.
    """
    out: Dict[str, Dict[str, float]] = {}
    try:
        chk = con.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='execution_capital_efficiency'"
        ).fetchone()
        if not chk:
            return {}
    except Exception:
    ...
    since = _now_ms() - int(max(1, int(lookback_days))) * 86400 * 1000

    try:
        rows = con.execute(
            """
            SELECT strategy_name,
                   (ts_ms / 86400000) as day_bucket,
                   SUM(COALESCE(notional,0)) as day_notional
            FROM execution_capital_efficiency
            WHERE ts_ms >= ?
              AND strategy_name IS NOT NULL
              AND notional IS NOT NULL
            GROUP BY strategy_name, day_bucket
            """,
            (int(since),),
        ).fetchall()
    except Exception:
    ...
    for sname, day_bucket, day_notional in rows or []:
        try:
            sn = str(sname or "").strip()
            if not sn:
                continue
            dn = float(day_notional or 0.0)
            if dn <= 0:
                continue
            buckets.setdefault(sn, []).append(float(dn))
            day_counts.setdefault(sn, set()).add(int(day_bucket or 0))
        except Exception:
            continue

    for sn, vals in buckets.items():
        if not vals:
            continue
        vals2 = sorted([float(v) for v in vals if v is not None])
        if not vals2:
            continue
        idx = int(round((len(vals2) - 1) * 0.80))
        idx = max(0, min(len(vals2) - 1, idx))
        out[str(sn)] = {
            "cap_notional_p80": float(vals2[idx]),
            "days": float(len(day_counts.get(sn) or set())),
        }

    return out


def _equity_snapshot_usd() -> Optional[float]:
    """Best-effort equity snapshot for converting weights->notional."""
    try:
        ...
        if snap and snap.get("ok"):
            ...
            if eq is not None:
                eqf = float(eq)
                return eqf if math.isfinite(eqf) and eqf > 0 else None
    except Exception:
        return None
    return None


def _load_avg_total_cost_bps_by_symbol(con, lookback_days: int) -> Dict[str, float]:
    """Loads avg realized total_cost_bps per symbol from labels_exec.

    This is net-of-cost execution quality, and is more robust than spread proxy.
    Fail-soft if labels_exec is missing.
    """
    out: Dict[str, float] = {}
    try:
        since = _now_ms() - int(max(1, int(lookback_days))) * 86400 * 1000
        rows = con.execute(
            """
            SELECT symbol, AVG(total_cost_bps) AS avg_cost
            FROM labels_exec
            WHERE ts_ms >= ?
              AND realized = 1
        ...
        for sym, avg_cost in rows or []:
            try:
                s = str(sym).upper().strip()
                if not s:
                    continue
                out[s] = float(avg_cost or 0.0)
            except Exception:
                continue
    except Exception:
        return {}
...
def _pick_best_per_symbol(alerts: List[Dict]) -> Dict[str, Dict]:
    ...
    for a in alerts:
        ...
        if PORTFOLIO_USE_HORIZON_PRIORITY:
            try:
                score *= float(horizon_priority(int(a.get("horizon_s") or 0)))
            except Exception:
                pass
        cur = best.get(sym)
...
def _apply_cost_aware_sizing(con, desired: Dict[str, Dict]) -> Dict[str, Dict]:
    """Penalize symbols with high realized *total* execution costs.

    Controlled via env:
      PORTFOLIO_COST_SIZING=1
      PORTFOLIO_COST_LOOKBACK_DAYS=30
      PORTFOLIO_COST_BAD_BPS=25
      PORTFOLIO_COST_FLOOR=0.25
    """
    if not desired or os.environ.get("PORTFOLIO_COST_SIZING", "1") != "1":
        return desired

    lookback_days = int(os.environ.get("PORTFOLIO_COST_LOOKBACK_DAYS", "30"))
    bad = float(os.environ.get("PORTFOLIO_COST_BAD_BPS", "25.0"))
    floor = float(os.environ.get("PORTFOLIO_COST_FLOOR", "0.25"))

    cost = _load_avg_total_cost_bps_by_symbol(con, int(lookback_days))
    if not cost:
        return desired

    for sym in list(desired.keys()):
        s = str(sym).upper().strip()
        cbps = float(cost.get(s, 0.0))
        if bad <= 1e-9:
            continue
        if cbps <= bad:
            desired[sym].setdefault("reason", {})
            desired[sym]["reason"]["cost_total_bps"] = float(cbps)
            desired[sym]["reason"]["cost_factor"] = 1.0
            continue

        raw = 1.0 - ((cbps / bad) - 1.0)
        f = max(float(floor), min(1.0, float(raw)))
        try:
            desired[sym]["weight"] = float(desired[sym].get("weight", 0.0) or 0.0) * float(f)
            desired[sym].setdefault("reason", {})
            desired[sym]["reason"]["cost_total_bps"] = float(cbps)
            desired[sym]["reason"]["cost_factor"] = float(f)
        except Exception:
            pass
...
def compute_rebalance() -> Dict:
    ...
    try:
        ...
        cap_map = _load_strategy_capacity(con, lookback_days=int(os.environ.get("STRATEGY_CAPACITY_LOOKBACK_DAYS", "30")))

        equity_usd = _equity_snapshot_usd()
        if equity_usd is None:
            equity_usd = float(os.environ.get("PORTFOLIO_EQUITY_PROXY", "1.0"))
        ...
        # Capacity-aware efficiency scaling (one-pass, deterministic)
        # If a strategy routinely saturates at cap_notional_p80, compress its effective efficiency
        # when current desired notional would exceed that cap.
        try:
            base_total = float(total_eff)
            if base_total <= 1e-9:
                base_total = float(len(strategy_targets) or 1)

            for sname, targets in (strategy_targets or {}).items():
                eff0 = float((eff_map.get(str(sname)) or {}).get("efficiency_score", 0.0) or 0.0)
                eff0 = max(0.0, eff0)
                share0 = float(eff0 / base_total) if base_total > 0 else 0.0

                # approximate desired notional = equity * gross_abs_weight * share
                gw = 0.0
                for _sym, tgt in (targets or {}).items():
                    try:
                        gw += abs(float((tgt or {}).get("weight", 0.0) or 0.0))
                    except Exception:
                        continue
                desired_notional = float(equity_usd) * float(gw) * float(share0)

                cap = float((cap_map.get(str(sname)) or {}).get("cap_notional_p80", 0.0) or 0.0)
                if cap > 0 and desired_notional > cap and desired_notional > 1e-9:
                    mult = float(cap) / float(desired_notional)
                    mult = max(0.10, min(1.0, mult))
                    # apply multiplier by mutating eff_map locally
                    eff_map.setdefault(str(sname), {})
                    eff_map[str(sname)]["capacity_mult"] = float(mult)
                    eff_map[str(sname)]["capacity_cap_notional_p80"] = float(cap)
                    eff_map[str(sname)]["capacity_desired_notional"] = float(desired_notional)
                    eff_map[str(sname)]["efficiency_score"] = float(eff0) * float(mult)
        except Exception:
            pass
...
# =========================
        ...
        # ---            -- ------------------------------------------------------
        # Cost-aware sizing (penalize symbols with high realized total execution cost)
        # ---            -- ------------------------------------------------------
        try:
            desired = _apply_cost_aware_sizing(con, desired)
        except Exception:
            pass
        ...
        try:
            ...
            if PORTFOLIO_USE_VOL_TARGET:
                ...
                for sym in list(desired.keys()):
                    vol = realized_vol_from_prices(con, sym, ts_ms=now_ms)
                    if vol is None:
                        continue
