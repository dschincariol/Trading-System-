...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            # honor external retraining signals if any
            forced = False
            try:
                ...
                if pending:
                    forced = True
                    print(f"embed_models: forced retrain due to signals={pending}")
                    for mname, reason, ts in pending:
                        mark_processed(mname, reason)
            except Exception:
                pass

            if (not forced) and ((not changed) or (new_labels < MIN_NEW_LABELS)):
                print(
