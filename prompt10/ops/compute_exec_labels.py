...
def _get_px_at_or_before(con, symbol: str, ts_ms: int) -> Optional[Tuple[float, int]]:
    # exclude the exact timestamp so that the price truly precedes the event
    r = con.execute(
        """
        SELECT ts_ms, price
        FROM prices
...
def main():
    ...
    with connect() as con:
        ...
        for eid, sym, horizon_s, ts_ms, predicted_z in rows:
            ...
            costs = estimate_cost_bps(px=px_in, bid=bid, ask=ask, side=side, horizon_s=int(horizon_s))
