...
def _rolling_z(con, symbol: str, horizon_s: int, new_ret: float, anchor_ts_ms: int) -> float:
    rows = con.execute(
        """
        SELECT ret
        FROM labels_price
        ...
        LIMIT ?
        """,
        (symbol, int(horizon_s), int(anchor_ts_ms), int(ZSCORE_LOOKBACK)),
    ).fetchall()
        ...
        try:
            ...
            for ts_pred_ms, sym in rows:
                ...
                for h in HORIZONS:
                    ...
                    ret_z = _rolling_z(con, sym, int(h), float(ret), int(ts_pred_ms))
