...
CONF_DECAY_HALF_LIFE_DAYS = float(os.environ.get("CONF_DECAY_HALF_LIFE_DAYS", "3.0"))

# stale embedding detection (days)
EMB_STALE_DAYS = float(os.environ.get("EMB_STALE_DAYS", "30"))

# optional retraining hook
...
def _load_labeled_event_vectors_cached():
    ...
    if (
        ...
        # stale embedding detection: if oldest event timestamp exceeds threshold, signal retrain
        try:
            if _cached.get("event_ts"):
                oldest_ts = min(_cached["event_ts"])
                age_days = (now_s * 1000 - oldest_ts) / MS_PER_DAY
                if age_days > EMB_STALE_DAYS:
                    schedule_retraining(model_name=os.environ.get("MODEL_NAME", "embed_regressor"), reason="stale_embeddings")
        except Exception:
            pass
...
def predict_event(
    ...
    for h in horizons:
        for sym in symbols:
            # prepare age-based confidence decay info
            def _label_age_days(symbol=sym, horizon_s=h):
                try:
                    con2 = connect()
                    row = con2.execute(
                        "SELECT MAX(created_at_ms) FROM labels WHERE symbol=? AND horizon_s=?",
                ...
                if not row or not row[0]:
                    return None
                return max(0.0, (time.time() * 1000 - int(row[0])) / MS_PER_DAY)
            ...
            if embed_pred is not None:
                ...
                age = _label_age_days()
                if age is not None:
                    decay = math.exp(-age / CONF_DECAY_HALF_LIFE_DAYS)
                    explain["conf_age_days"] = float(age)
                    explain["conf_decay"] = float(decay)
                    conf2 = float(max(0.0, min(1.0, conf2 * decay)))
                    # if decay severe, schedule retrain
                    if decay < 0.5:
                        try:
                            schedule_retraining(model_name=os.environ.get("MODEL_NAME", "embed_regressor"), reason="confidence_decay")
                        except Exception:
                            pass
            ...
            if age is not None:
                ...
                explain["conf_age_days"] = float(age)
                explain["conf_decay"] = float(decay)
                conf = float(max(0.0, min(1.0, conf * decay)))
                if decay < 0.5:
                    try:
                        schedule_retraining(model_name=os.environ.get("MODEL_NAME", "embed_regressor"), reason="confidence_decay")
                    except Exception:
