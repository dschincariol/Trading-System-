...
export function detectExecutionDegradation(rows, toastFn) {
  ...
  const stateRaw = localStorage.getItem(EXEC_CONF_STATE_KEY);
  ...
  localStorage.setItem(EXEC_CONF_STATE_KEY, JSON.stringify(state));
  return alerts;
}
...
export function isExecutionDegraded() {
  const raw = localStorage.getItem(EXEC_CONF_STATE_KEY);
  if (!raw) return false;
