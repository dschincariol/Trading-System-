...
def _get_last_price_ts_ms(con, symbol: str, ts_ms: Optional[int] = None) -> Optional[int]:
    # restrict to prices strictly before as-of timestamp when provided
    if ts_ms is None:
        row = con.execute("SELECT MAX(ts_ms) FROM prices WHERE symbol=?", (symbol,)).fetchone()
    else:
        row = con.execute(
            "SELECT MAX(ts_ms) FROM prices WHERE symbol=? AND ts_ms < ?",
            (symbol, int(ts_ms)),
        ).fetchone()
    if not row:
        return None
...
def _vol_proxy_multiplier(con, symbol: str, ts_ms: int) -> Tuple[float, Dict[str, Any]]:
    ...
    # Map vol_proxy to penalty in [0, VOL_MAX_PENALTY]
    # 0.0 -> 0 penalty
    # 0.02 -> ~half penalty
    # 0.05+ -> near max penalty
...
def get_adjusted_confidence(
    ...
    last_ts = _get_last_price_ts_ms(con, sym, ts_ms=now_ms)
    if last_ts is None:
