...
def _ensure_schemas() -> List[str]:
    notes: List[str] = []
    from engine.storage import init_db, connect
    from engine.alerts import init_alerts_db
    from engine.execution_ledger import init_execution_ledger
...
def _verify_news_layer() -> Tuple[List[str], List[str]]:
    notes: List[str] = []
    warnings: List[str] = []
    from engine.storage import connect
    con = connect()
    try:
        for t in ("news_features", "news_symbol_embeddings", "domain_perf", "domain_blacklist"):
            if not _table_exists(con, t):
                warnings.append(f"news layer table missing: {t}")

        if _table_exists(con, "news_features"):
            try:
                row = con.execute(
                    "SELECT COUNT(*) FROM news_features WHERE bucket_ts_ms >= (strftime('%s','now')*1000 - 86400000)"
                ).fetchone()
                n = int(row[0] or 0) if row else 0
                if n <= 0:
                    warnings.append("news_features has no rows in the last 24h")
                else:
                    notes.append(f"news_features recent_rows={n}")
            except Exception as e:
                warnings.append(f"news_features check failed: {e}")
    finally:
        try:
            con.close()
        except Exception:
...
def main() -> int:
    ...
    nnotes, nwarns = _verify_news_layer()
    result["steps"].extend(nnotes)
    result["warnings"].extend(nwarns)
