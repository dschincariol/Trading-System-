...
# ------            -- ------------------------------------------------------
def _net_eval_metrics(con, lookback_days: int = 90, horizon_s: Optional[int] = None) -> Optional[Dict[str, Any]]:
    DAY_MS = 86400 * 1000
...
def _load_embed_eval_rows(con, ts_ms: int):
    return con.execute(
        """
        SELECT horizon_s, model_kind, n_eval, rmse, directional_acc
...
def _aggregate(rows) -> Tuple[str, Dict[str, Any]]:
    ...
    for _h, mk, n_eval, rmse, diracc in rows:
        w = float(n_eval or 0)
        if w <= 0:
            continue
...
def _aggregate_for_horizon(rows, horizon_s: int) -> Tuple[str, Dict[str, Any]]:
    h = int(horizon_s)
    sub = [r for r in (rows or []) if int(r[0] or 0) == h]
    kind, m = _aggregate(sub)
    m = dict(m or {})
    m["horizon_s"] = int(h)
    return kind, m


def _hkey(regime: str, horizon_s: int) -> str:
    return f"{str(regime)}|h={int(horizon_s)}"


def _promote_if_beats(
    *,
    model_name: str,
    model_kind: str,
    model_ts_ms: int,
    ...
    guard_reason: Dict[str, Any],
    actor: str,
    cmp_reason_global: Dict[str, Any],
) -> None:
    champ = get_stage_latest(model_name, "champion", regime=str(key))
    ok_h, cmp_h = _beats_champion(candidate_metrics, champ)
    if not ok_h:
        audit(
            actor=str(actor),
            action="reject",
            model_name=str(model_name),
            from_kind=(champ.get("model_kind") if champ else None),
            from_ts_ms=(champ.get("model_ts_ms") if champ else None),
            to_kind=str(model_kind),
            to_ts_ms=int(model_ts_ms),
            reason={"compare": cmp_h, "guard": guard_reason, "metrics": candidate_metrics, "scope": "horizon"},
            regime=str(key),
        )
        return
    ...
    promote_with_snapshot_and_db_watch(
        ...
        actor=str(actor),
        extra_reason={
            "compare_global": cmp_reason_global,
            "compare_horizon": cmp_h,
            "guard": guard_reason,
...
def main() -> int:
    ...
    try:
        ...
        # 4.1) Register challenger per horizon (independent promotion tracks)
        hs = horizons_s()
        for h in hs:
            h_kind, h_metrics = _aggregate_for_horizon(rows, int(h))

            # net-of-cost for this horizon (best-effort)
            con = connect()
            try:
                h_nem = _net_eval_metrics(con, lookback_days=90, horizon_s=int(h))
            finally:
                con.close()
            if h_nem:
                h_metrics.update(h_nem)
            ...
            register_model(
                ...
                model_ts_ms=int(snap),
                stage="challenger",
                regime=_hkey("global", int(h)),
                metrics=h_metrics,
                note="pipeline_train_and_eval_horizon",
            )
        # -----------------------------
        ...
        for regime in ACTIVE_REGIMES:
            ...
            # Horizon-specific promotions (independent)
            for h in horizons_s():
                hkey = _hkey(str(regime), int(h))

                # Ensure (regime,horizon) has enough labels
                try:
                    con = connect()
                    try:
                        row = con.execute(
                ...
                min_rh = int(os.environ.get("PROMOTE_MIN_REGIME_HORIZON_LABELS", "50"))
                if n_rh < min_rh:
                    continue

                h_kind, h_metrics = _aggregate_for_horizon(rows, int(h))

                # Optional net-of-cost evaluation for this horizon
                con = connect()
                try:
                    h_nem = _net_eval_metrics(con, lookback_days=90, horizon_s=int(h))
                finally:
                ...
                _promote_if_beats(
                    model_name=MODEL_NAME,
                    model_kind=str(h_kind),
                    model_ts_ms=int(snap),
                    key=str(hkey),
                    candidate_metrics=h_metrics,
                    guard_reason=guard_reason,
                    actor="auto",
                    cmp_reason_global=cmp_reason,
                )
