...
# news embedding helpers (symbol-aware vectors)
from engine.data.news_embeddings import ensure_symbol_embedding
    ...
    try:
        ...
        try:
            ...
            for (eid, ts_ms, source, title, body, url, meta_json), vec in zip(rows, embeddings):
                ...
                # events that were previously tagged as quarantined should be
                # bypassed; we update the checkpoint to avoid infinite loops.
                ...
                for sym in symbols:
                    # compute / cache a symbol-aware embedding for later use
                    try:
