...
def train_regime_stats(symbols: List[str], horizons: List[int], lookback_days: int = 90) -> int:
    ...
    with connect() as con:
        ...
        con.commit()
        # adaptive retraining: if we updated any regime stats, signal embedding retrain
        try:
            if updated > 0:
                from engine.strategy.training_hooks import schedule_retraining
                schedule_retraining(model_name=os.environ.get("MODEL_NAME", "embed_regressor"), reason="regime_stats_updated")
        except Exception:
            pass
        return int(updated)
