...
def main():
    ...
    for i, title in enumerate(NEWS):
        ...
        # attach minimal metadata (source) so that later training jobs can
        # attribute performance back to this synthetic feed.
        meta = json.dumps({"source": "dev"})
        eid = safe_db_operation(lambda conn: conn.execute(
            "INSERT OR REPLACE INTO events(ts_ms, source, title, body, url, event_key, meta_json) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (ts, "dev", title, "", "", f"dev:{i}:{abs(hash(title)) % 10_000_000}", meta)
        ) or conn.lastrowid)
